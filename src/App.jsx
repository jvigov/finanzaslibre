...
const sb = {
  async select(table, query, tok) {
    const r = await fetch(SUPA_URL+"/rest/v1/"+table+"?"+(query||"select=*"), { headers:h(tok) });
    return r.json();
  },
  async insert(table, body, tok) {
    const r = await fetch(SUPA_URL+"/rest/v1/"+table, { method:"POST", headers:{...h(tok),"Prefer":"return=representation"}, body:JSON.stringify(body) });
    const d = await r.json(); return Array.isArray(d)?d[0]:d;
  },
  async update(table, id, body, tok) {
    await fetch(SUPA_URL+"/rest/v1/"+table+"?id=eq."+id, { method:"PATCH", headers:{...h(tok),"Prefer":"return=minimal"}, body:JSON.stringify(body) });
  },
  async delete(table, id, tok) {
    await fetch(SUPA_URL+"/rest/v1/"+table+"?id=eq."+id, { method:"DELETE", headers:h(tok) });
  },
  async signUp(email, pwd, nombre) {
    const r = await fetch(SUPA_URL+"/auth/v1/signup", { method:"POST", headers:{"Content-Type":"application/json","apikey":SUPA_KEY}, body:JSON.stringify({ email, password:pwd, options:{ data:{ nombre } } }) });
    return r.json();
  },
  async signIn(email, pwd) {
    const r = await fetch(SUPA_URL+"/auth/v1/token?grant_type=password", { method:"POST", headers:{"Content-Type":"application/json","apikey":SUPA_KEY}, body:JSON.stringify({ email, password:pwd }) });
    return r.json();
  },
};
...
const C = {
  bg:"#060c15", surface:"#0a1520", card:"#0e1c2c", card2:"#122030",
  border:"#172538", accent:"#0ea5e9", emerald:"#10b981", danger:"#f43f5e",
  warn:"#f59e0b", gold:"#fbbf24", purple:"#8b5cf6",
  text:"#e0eaf8", soft:"#7a9bb8", muted:"#3d5570",
};
const S = {
...
function Modal({ title, onClose, children }) {
  return (
    <div style={{ position:"fixed", inset:0, background:"#000c", zIndex:300, display:"flex", alignItems:"center", justifyContent:"center", padding:16 }}>
      <div style={{ background:C.card, border:"1px solid "+C.border, borderRadius:20, padding:22, width:"100%", maxWidth:480, maxHeight:"94vh", overflowY:"auto" }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:18 }}>
          <span style={{ fontWeight:800, fontSize:15 }}>{title}</span>
          <button onClick={onClose} style={{ background:"none", border:"none", color:C.muted, fontSize:24, cursor:"pointer" }}>x</button>
        </div>
        {children}
      </div>
    </div>
...
function AuthScreen({ onLogin }) {
  ...
  const login = async () => {
    ...
    try {
      const d = await sb.signIn(email, pwd);
      if (d.access_token) onLogin({ user:d.user, token:d.access_token, nombre:d.user.user_metadata?.nombre||email.split("@")[0] });
      else setError(d.error_description||d.msg||"Email o contrasena incorrectos");
  ...
  const registro = async () => {
    ...
    setLoading(true); setError("");
    try {
      const d = await sb.signUp(email, pwd, nombre);
      if (d.error) { setError(d.error_description||d.msg||JSON.stringify(d.error)); setLoading(false); return; }
      const d2 = await sb.signIn(email, pwd);
      if (d2.access_token) onLogin({ user:d2.user, token:d2.access_token, nombre });
      else setError("Cuenta creada. Desactiva confirmacion de email en Supabase y vuelve a intentar.");
    } catch(e) { setError("Error: "+e.message); }
    setLoading(false);
  };
    ...
    <div style={{ minHeight:"100vh", background:C.bg, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}>
      <div style={{ width:"100%", maxWidth:400 }}>
        <div style={{ textAlign:"center", marginBottom:32 }}>
          <div style={{ fontSize:30, fontWeight:900 }}><span style={{ color:C.accent }}>Finanzas</span><span style={{ color:C.text }}>Libre</span></div>
          <div style={{ fontSize:13, color:C.muted, marginTop:4 }}>Tu camino a la libertad financiera</div>
        </div>
        <div style={{ display:"flex", background:C.surface, borderRadius:12, padding:4, marginBottom:20, border:"1px solid "+C.border }}>
          {[["login","Iniciar sesion"],["registro","Crear cuenta"]].map(([k,l])=>(
            <button key={k} onClick={()=>{ setModo(k); setError(""); }}
              style={{ flex:1, padding:"10px", borderRadius:9, border:"none", background:modo===k?C.card:"transparent", color:modo===k?C.text:C.muted, fontWeight:700, fontSize:13, cursor:"pointer" }}>{l}</button>
          ))}
        </div>
        <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
          ...
          <button style={{ ...S.btn(C.accent), width:"100%", padding:"13px", fontSize:14, opacity:loading?0.7:1 }} onClick={modo==="login"?login:registro} disabled={loading}>
...
- fecha: formato YYYY-MM-DD. Si dice "ayer" resta 1 dia, "lunes" calcula el lunes pasado, "el 5 de mayo" seria 2026-05-05, etc.
- cat para gastos: Alimentacion, Comida afuera, Compras casa, Taxi, Transporte, Salud, Educacion, Suscripciones, Entretenimiento, Ropa, Deuda cuota, Negocio, Otro
- cat para ingresos: Sueldo, CTS, Gratificacion, Bono, Freelance, Negocio ventas, Multinivel cheque, Otro ingreso
- confianza: "alta" si el monto es claro, "media" si es aproximado, "baja" si hay duda
- Si no hay monto claro igual intenta extraer lo que puedas`;

  const analizar = async (txt, b64) => {
    setAnaliz(true); setError(""); setResultado(null);
    try {
      const prompt = b64
        ? `Analiza este comprobante/recibo y extrae los datos del gasto. Hoy es ${today()}. Responde UNICAMENTE con JSON: {"tipo":"gasto","monto":0,"cat":"Otro","descripcion":"descripcion del recibo","fecha":"${today()}","fuente":"Personal","confianza":"alta"}`
        : PROMPT(txt);
      const resp = await callClaude(prompt, b64);
      const clean = resp.replace(/```json|```/g,"").trim();
      const start = clean.indexOf("{");
      const end = clean.lastIndexOf("}");
      const parsed = JSON.parse(clean.slice(start, end+1));
      setResultado({...parsed, id:Date.now()});
    } catch(e) {
      setError("No pude interpretar el mensaje. Intenta de nuevo.");
    }
    setAnaliz(false);
  };
  ...
  const iniciarGrabacion = async () => {
    try {
      ...
      rec.onstop = async () => {
        ...
        setAnaliz(true); setError("");
        try {
          const blob = new Blob(chunks, { type:"audio/webm" });
          const reader = new FileReader();
          reader.onload = async () => {
            const b64 = reader.result.split(",")[1];
            const prompt = `Transcribe este audio y extrae los datos financieros. Hoy es ${today()}. El usuario habla en espanol peruano y puede decir cosas como "gaste 38 en taxi", "me pagaron 3000 de sueldo", etc. Responde UNICAMENTE con JSON: {"tipo":"gasto","monto":0,"cat":"Otro","descripcion":"lo que dijo","fecha":"${today()}","fuente":"Personal","confianza":"alta","transcripcion":"texto del audio"}`;
        ...
        } catch(e) { setError("No pude procesar el audio. Intenta de nuevo."); setAnaliz(false); }
      };
      rec.start();
      setMediaRec(rec);
      setGrabando(true);
    } catch(e) { setError("No se pudo acceder al microfono. Permite el permiso."); }
  };
  ...
  const confirmar = async () => {
    if(!resultado) return;
    const body = { user_id:userId, tipo:resultado.tipo, cat:resultado.cat, monto:resultado.monto, descripcion:resultado.descripcion, fecha:resultado.fecha, fuente:resultado.fuente||"Personal" };
    const data = await sb.insert("transacciones", body, sesion.token);
    if(data&&data.id) { setTxns(p=>[data,...p]); setResultado(null); setInput(""); setImgB64(null); setImgPrev(null); setGuardado(true); setTimeout(()=>setGuardado(false),2500); }
  ...
  const guardarManual = async () => {
    ...
    const data = await sb.insert("transacciones", body, sesion.token);
    if(data&&data.id) { setTxns(p=>[data,...p]); setMForm({ tipo:"gasto", cat:"Alimentacion", monto:"", descripcion:"", fecha:today(), fuente:"Personal" }); setGuardado(true); setTimeout(()=>setGuardado(false),2500); }
  };
  ...
  const cats = {}; filtradas.filter(t=>t.tipo==="gasto").forEach(t=>{ cats[t.cat]=(cats[t.cat]||0)+t.monto; });
  ...
  return (
    <div>
      <div style={{ display:"flex", gap:6, marginBottom:16 }}>
        {[["ia","Texto"],["foto","Foto"],["voz","Voz"],["manual","Manual"]].map(([k,l])=>(
          <button key={k} onClick={()=>{ setModo(k); setResultado(null); setError(""); }}
            style={{ flex:1, padding:"9px 4px", borderRadius:10, border:"1px solid "+(modo===k?C.accent:C.border), background:modo===k?C.accent+"18":C.surface, color:modo===k?C.accent:C.muted, fontWeight:700, fontSize:11, cursor:"pointer" }}>{l}</button>
        ))}
      </div>

      {modo==="ia" && (
        <div style={{ marginBottom:14 }}>
          <textarea style={{ ...S.inp, height:100, resize:"none", lineHeight:1.6 }} value={input} onChange={e=>setInput(e.target.value)}
            placeholder={"Escribe como quieras, por ejemplo:\n- 7.7 soles taxi el 05 de mayo\n- ayer almorce 22 en el chifa\n- me pagaron 3000 de sueldo hoy"}/>
          {!resultado && <button style={{ ...S.btn(C.accent), width:"100%", marginTop:10, padding:"13px", fontSize:14, opacity:analiz?0.7:1 }}
            onClick={()=>analizar(input)} disabled={analiz||!input.trim()}>
            {analiz?"Procesando...":"Registrar con IA"}
          </button>}
        </div>
      )}
      ...
      {modo==="voz" && (
        <div style={{ textAlign:"center", padding:"24px 0", marginBottom:14 }}>
          <div style={{ fontSize:13, color:C.soft, marginBottom:20 }}>
            {grabando?"Grabando... habla claro y toca para terminar":"Toca el microfono y di lo que gastaste o recibiste"}
          </div>
          <button
            onClick={grabando?detenerGrabacion:iniciarGrabacion}
            style={{ width:90, height:90, borderRadius:"50%", border:"none", background:grabando?C.danger:C.accent, color:"#040a12", fontSize:36, cursor:"pointer", display:"block", margin:"0 auto 16px", boxShadow:grabando?"0 0 0 8px "+C.danger+"40":"none", transition:"all .3s" }}>
            {analiz?"...":grabando?"⏹":"🎤"}
          </button>
          {grabando && <div style={{ color:C.danger, fontWeight:700 }}>Grabando... toca para detener</div>}
          {analiz && <div style={{ color:C.accent, fontSize:12, marginTop:10 }}>Procesando audio con IA...</div>}
          <div style={{ fontSize:11, color:C.muted, marginTop:16 }}>
            Ejemplos: "Taxi 38 soles hoy", "Almorce 22 ayer", "Me pagaron 3000"
          </div>
        </div>
      )}
      ...
      {modo==="foto" && (
        <div style={{ marginBottom:14 }}>
          <label style={{ display:"block", border:"2px dashed "+C.border, borderRadius:14, padding:20, textAlign:"center", cursor:"pointer", background:C.surface, marginBottom:10 }}>
            {imgPrev?<img src={imgPrev} style={{ maxWidth:"100%", maxHeight:180, borderRadius:10, objectFit:"contain" }}/>:
              <div><div style={{ fontSize:36, marginBottom:8 }}>foto</div><div style={{ color:C.soft, fontSize:13 }}>Toca para sacar foto o elegir de galeria</div></div>}
            <input type="file" accept="image/*" capture="environment" onChange={handleFoto} style={{ display:"none" }}/>
          </label>
          {imgB64&&!resultado&&<button style={{ ...S.btn(C.accent), width:"100%", padding:"12px", opacity:analiz?0.7:1 }}
            onClick={()=>analizar("",imgB64)} disabled={analiz}>{analiz?"Analizando...":"Extraer datos del recibo"}</button>}
        </div>
      )}
      ...
      {modo==="manual" && (
        <div style={{ marginBottom:14 }}>
          <div style={{ display:"flex", gap:8, marginBottom:12 }}>
            {["gasto","ingreso"].map(t=>(
              <button key={t} onClick={()=>uf("tipo",t)}
                style={{ flex:1, padding:"10px", borderRadius:10, border:"1px solid "+(t===mForm.tipo?(t==="gasto"?C.danger:C.emerald):C.border), background:t===mForm.tipo?(t==="gasto"?C.danger+"20":C.emerald+"20"):C.surface, color:t===mForm.tipo?(t==="gasto"?C.danger:C.emerald):C.muted, fontWeight:700, cursor:"pointer" }}>
          ...
          <button style={{ ...S.btn(mForm.tipo==="ingreso"?C.emerald:C.danger), width:"100%", padding:"12px" }} onClick={guardarManual}>Guardar</button>
        </div>
      )}

      {error&&<div style={{ ...S.alert(C.danger), marginBottom:10 }}>{error}</div>}

      {resultado&&(
        <div style={{ ...S.card, border:"1px solid "+C.emerald+"50", marginBottom:14 }}>
          <div style={{ fontWeight:800, marginBottom:12, color:C.emerald }}>La IA detecto:</div>
          <div style={S.g2}>
            <div style={S.box(resultado.tipo==="ingreso"?C.emerald:C.danger)}><div style={S.bv(resultado.tipo==="ingreso"?C.emerald:C.danger,18)}>{fmt(resultado.monto)}</div><div style={S.bl}>{resultado.tipo}</div></div>
            <div style={S.box(C.accent)}><div style={{ fontWeight:700, fontSize:14, color:C.accent }}>{resultado.cat}</div><div style={S.bl}>Categoria</div></div>
          </div>
          <div style={{ fontSize:13, marginTop:12, marginBottom:4 }}><span style={{ color:C.muted }}>Descripcion: </span><b>{resultado.descripcion}</b></div>
          <div style={{ fontSize:13, marginBottom:14 }}><span style={{ color:C.muted }}>Fecha: </span><b>{resultado.fecha}</b></div>
          <div style={{ display:"flex", gap:8 }}>
            <button style={{ ...S.btn(C.emerald), flex:2, padding:"12px" }} onClick={confirmar}>Confirmar y guardar</button>
            <button style={{ ...S.btn(C.danger), flex:1, padding:"12px" }} onClick={()=>setResultado(null)}>Corregir</button>
          </div>
          <div style={{ fontSize:10, color:C.muted, marginTop:8, textAlign:"center" }}>Confianza: <b style={{ color:resultado.confianza==="alta"?C.emerald:C.warn }}>{resultado.confianza}</b></div>
        </div>
      )}

      {guardado&&<div style={{ ...S.alert(C.emerald), textAlign:"center", marginBottom:10 }}><b>Guardado correctamente</b></div>}

      <div style={{ display:"flex", gap:8, marginBottom:12, marginTop:8 }}>
        <input type="month" style={{ ...S.inp, flex:1 }} value={mes} onChange={e=>setMes(e.target.value)}/>
      </div>
      <div style={S.g3}>
        <div style={S.box(C.emerald)}><div style={S.bv(C.emerald,16)}>{fmtK(ingMes)}</div><div style={S.bl}>Ingresos</div></div>
        <div style={S.box(C.danger)}><div style={S.bv(C.danger,16)}>{fmtK(gastMes)}</div><div style={S.bl}>Gastos totales</div></div>
      ...
      <div style={S.card}>
        <div style={{ display:"flex", justifyContent:"space-between", marginBottom:8 }}>
          <span style={{ fontWeight:700, fontSize:12 }}>Fijos automaticos del mes</span>
          <span style={{ fontWeight:800, color:C.danger }}>-{fmt(TOTAL_FIJOS)}</span>
        </div>
        {GASTOS_FIJOS.map(g=>(
      ...
      {catsArr.length>0&&<div style={S.card}>
        <div style={S.lbl}>Variables por categoria</div>
        {catsArr.map(([cat,monto])=>(
          <div key={cat} style={{ marginBottom:9 }}>
            <div style={{ display:"flex", justifyContent:"space-between", marginBottom:3 }}>
              <span style={{ fontSize:12 }}>{cat}</span>
              <span style={{ fontSize:12, fontWeight:700 }}>{fmt(monto)} <span style={{ color:C.muted }}>({gastVar>0?((monto/gastVar)*100).toFixed(0):0}%)</span></span>
            </div>
            <div style={S.bar()}><div style={S.fill(gastVar>0?(monto/gastVar)*100:0,C.accent)}/></div>
          </div>
        ))}
      </div>}

      {dias.length===0&&<div style={{ textAlign:"center", padding:30, color:C.muted }}>
        <div style={{ fontSize:14 }}>Sin movimientos registrados</div>
        <div style={{ fontSize:12, marginTop:6 }}>Usa IA, foto o manual para registrar</div>
      </div>}
...
function TabDashboard({ perfil, deudas, txns, extras }) {
  ...
  const NIVELES=[
    ...
    {n:1,label:"Sobreviviendo",color:"#f97316",desc:"Ingresos igual a gastos"},
    ...
    {n:5,label:"Ingresos pasivos 25%",color:C.accent,desc:"Empiezas a construir libertad"},
  ...
  return (
    <div>
      {criticas.map(d=>(
        <div key={d.id} style={S.alert(C.danger)}>
          <div style={{ fontWeight:800, marginBottom:3 }}>Urgente: {d.nombre} - {d.dias_atraso} dias vencido</div>
          <div style={{ fontSize:12 }}>SBS: Perdida. Negociar urgente.</div>
        </div>
      ))}
      <div style={{ ...S.card, background:"linear-gradient(135deg,"+niv.color+"15,"+C.card+")", border:"1px solid "+niv.color+"35" }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:14, marginBottom:12 }}>
          ...
          {nAct<7&&<div style={{ marginLeft:"auto", textAlign:"right" }}>
            <div style={{ fontSize:10, color:C.muted }}>Siguiente</div>
            <div style={{ fontSize:11, fontWeight:600, color:NIVELES[nAct+1].color }}>{NIVELES[nAct+1].label}</div>
          </div>}
        </div>
      ...
      <div style={S.card}>
        ...
        <div style={{ marginBottom:10 }}>
          <div style={{ display:"flex", justifyContent:"space-between", marginBottom:4 }}>
            <span style={{ fontSize:12 }}>Cuotas / ingresos</span>
            <span style={{ fontWeight:700, color:ratioDeuda>0.4?C.danger:C.warn }}>{(ratioDeuda*100).toFixed(0)}% (meta: -30%)</span>
          </div>
          <div style={S.bar()}><div style={S.fill(ratioDeuda*100,ratioDeuda>0.4?C.danger:C.warn)}/></div>
        </div>
        <div style={S.bar()}><div style={S.fill((hoy/diasMes)*100,C.accent)}/></div>
      </div>
      {extras.filter(e=>e.mes>=curMes()).length>0&&<div style={S.card}>
        <div style={S.lbl}>Proximos extraordinarios</div>
        {extras.filter(e=>e.mes>=curMes()).slice(0,4).map(e=>(
          <div key={e.id} style={{ padding:"9px 0", borderBottom:"1px solid "+C.border }}>
            <div style={{ display:"flex", justifyContent:"space-between" }}>
              <span style={{ fontWeight:700 }}>{e.mes}</span>
              <span style={{ fontWeight:900, color:C.gold }}>{fmt(e.monto)}</span>
            </div>
            <div style={{ fontSize:11, color:C.muted }}>{e.concepto}{e.accion?" - "+e.accion:""}</div>
          </div>
        ))}
      </div>}
      {perfil.eirl>0&&<div style={{ ...S.card, border:"1px solid "+C.purple+"35" }}>
        <div style={S.lbl}>Cuenta EIRL (protegida)</div>
        <div style={{ fontWeight:900, fontSize:22, color:C.purple }}>{fmt(perfil.eirl)}</div>
        <div style={{ fontSize:11, color:C.muted, marginTop:4 }}>Capital de trabajo Fuxion. Aislada.</div>
      </div>}
      ...
      {perfil.meta&&<div style={{ ...S.card, border:"1px solid "+C.gold+"30" }}>
        <div style={S.lbl}>Tu meta</div>
        <div style={{ fontSize:13, color:C.soft, fontStyle:"italic" }}>"{perfil.meta}"</div>
      </div>}
    </div>
  );
}
...
function TabDeudas({ userId, sesion, deudas, setDeudas }) {
  ...
  const guardar = async () => {
    ...
    if (f.id) {
      await sb.update("deudas", f.id, body, sesion.token);
      setDeudas(p=>p.map(d=>d.id===f.id?{...body,id:f.id}:d));
    } else {
      const data = await sb.insert("deudas", body, sesion.token);
      if(data&&data.id) setDeudas(p=>[...p,data]);
    }
    setModal(null);
  };
  ...
  return (
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 }}>
        <div>
          <div style={{ fontWeight:900, fontSize:22, color:C.danger }}>{fmt(total)}</div>
          <div style={{ fontSize:11, color:C.muted }}>Cuotas: {fmt(cuotas)}/mes - {deudas.length} deudas</div>
        </div>
          ...
          <div key={d.id} style={{ ...S.card, borderLeft:"3px solid "+color }}>
            <div style={{ display:"flex", justifyContent:"space-between", marginBottom:8 }}>
              <div style={{ flex:1, minWidth:0, paddingRight:10 }}>
                ...
                {d.nota&&<div style={{ fontSize:11, color:C.warn, marginTop:3 }}>{d.nota}</div>}
              </div>
      ...
      {modal&&<Modal title={modal==="new"?"Nueva deuda":"Editar deuda"} onClose={()=>setModal(null)}>
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:10 }}>
          ...
          <div><label style={S.lbl2}>Tipo</label><select style={S.sel} value={f.tipo} onChange={e=>uf("tipo",e.target.value)}>
            {["Prestamo personal","Tarjeta credito","Prestamo PYME","Hipoteca","Deuda personal","Otro"].map(t=><option key={t}>{t}</option>)}
          </select></div>
...
function TabCobrar({ userId, sesion, cobrar, setCobrar }) {
  ...
  const abonar = async (c) => {
    const m = prompt("Cuanto abono "+c.deudor+"?"); if(!m||+m<=0) return;
    const nuevo=(c.abonado||0)+(+m); const estado=nuevo>=c.monto?"cobrado":"parcial";
  ...
  return (
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 }}>
        <div>
          <div style={{ fontWeight:900, fontSize:22, color:C.emerald }}>{fmt(total)}</div>
          <div style={{ fontSize:11, color:C.muted }}>Por cobrar - {cobrar.filter(c=>c.estado!=="cobrado").length} persona(s)</div>
        </div>
          ...
          <div key={c.id} style={{ ...S.card, borderLeft:"3px solid "+color }}>
            <div style={{ display:"flex", justifyContent:"space-between", marginBottom:8 }}>
              <div>
                ...
                {c.vencimiento&&<div style={{ fontSize:11, color:venc?C.danger:C.muted }}>Vence: {c.vencimiento}{venc?" VENCIDO":""}</div>}
              </div>
              <div style={{ textAlign:"right" }}>
                <div style={{ fontWeight:900, fontSize:18, color:C.emerald }}>{fmt(pend)}</div>
                <div style={{ fontSize:11, color:C.muted }}>de {fmt(c.monto)}</div>
              </div>
            </div>
            {c.abonado>0&&<div style={{ marginBottom:10 }}>
              <div style={{ display:"flex", justifyContent:"space-between", marginBottom:3 }}>
                <span style={{ fontSize:11, color:C.muted }}>Cobrado: {fmt(c.abonado)}</span>
                <span style={{ fontSize:11, fontWeight:700 }}>{c.monto>0?((c.abonado/c.monto)*100).toFixed(0):0}%</span>
              </div>
              <div style={S.bar()}><div style={S.fill(c.monto>0?(c.abonado/c.monto)*100:0,C.emerald)}/></div>
            </div>}
      ...
      {cobrar.filter(c=>c.estado==="cobrado").length>0&&<div style={S.card}>
        ...
        {cobrar.filter(c=>c.estado==="cobrado").map(c=>(
          <div key={c.id} style={{ display:"flex", justifyContent:"space-between", padding:"8px 0", borderBottom:"1px solid "+C.border }}>
            ...
            <div style={{ display:"flex", gap:8, alignItems:"center" }}>
              <span style={{ fontWeight:700, color:C.emerald }}>{fmt(c.monto)}</span>
...
function TabPlan({ perfil, deudas, extras }) {
  ...
  const FASES=[
    {n:1,tit:"Apagar incendios",sub:"Ahora",c:C.danger,activo:deudas.some(d=>d.dias_atraso>60),items:["Negociar deudas vencidas","Detener nuevas deudas","Usar CTS estrategicamente"]},
  ...
  return (
    <div>
      ...
      {totalDeuda>0&&<div style={S.card}>
        <div style={S.lbl}>Proyeccion salida de deudas</div>
        <div style={S.g2}>
          <div style={S.box(C.warn)}><div style={S.bv(C.warn)}>{m1>0?(m1/12).toFixed(1):"inf"} anos</div><div style={S.bl}>Solo cuotas</div></div>
          <div style={S.box(C.emerald)}><div style={S.bv(C.emerald)}>{m2>0?(m2/12).toFixed(1):"inf"} anos</div><div style={S.bl}>+{fmtK(ext)}/mes extra</div></div>
        </div>
        {ext>0&&<div style={{ fontSize:11, color:C.muted, marginTop:8 }}>Ahorras {m1-m2} meses pagando {fmt(ext)} extra/mes.</div>}
      </div>}
      {extras.length>0&&<div style={S.card}>
        <div style={S.lbl}>Calendario estrategico</div>
        {extras.map(e=>(
          <div key={e.id} style={{ padding:"9px 0", borderBottom:"1px solid "+C.border }}>
            <div style={{ display:"flex", justifyContent:"space-between" }}>
              <span style={{ fontWeight:700 }}>{e.mes}</span>
      ...
      <div style={S.card}>
        <div style={S.lbl}>Tu hoja de ruta</div>
        {FASES.map((f,i)=>{
          const esA=i===faseActual, comp=faseActual>=0&&i<faseActual;
          return (
            <div key={f.n} style={{ display:"flex", gap:14, marginBottom:18 }}>
              ...
              <div style={{ flex:1, paddingBottom:6 }}>
                <div style={{ display:"flex", gap:8, alignItems:"center", flexWrap:"wrap", marginBottom:5 }}>
                  <span style={{ fontWeight:800, color:esA?f.c:C.text, fontSize:13 }}>{f.tit}</span>
                  {esA&&<span style={S.tag(f.c)}>Aqui estas</span>}
                  <span style={{ fontSize:10, color:C.muted, marginLeft:"auto" }}>{f.sub}</span>
                </div>
...
export default function App() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem("fl_tok");
    const savedUser = localStorage.getItem("fl_user");
    if (saved && savedUser) setSesion({ token:saved, user:JSON.parse(savedUser), nombre:JSON.parse(savedUser).user_metadata?.nombre||"" });
  }, []);
  ...
  const onLogin = (s) => {
    localStorage.setItem("fl_tok", s.token);
    localStorage.setItem("fl_user", JSON.stringify(s.user));
    setSesion(s);
  };
  const onLogout = () => {
    localStorage.removeItem("fl_tok"); localStorage.removeItem("fl_user");
    setSesion(null); setPerfil({}); setDeudas([]); setTxns([]); setCobrar([]); setExtras([]);
  };
  ...
  return (
    <div style={S.app}>
      ...
      <nav style={S.nav}>
        {TABS.map((t,i)=><button key={i} style={S.nb(tab===i)} onClick={()=>setTab(i)}>{t}</button>)}
      </nav>
      {loading&&<div style={{ textAlign:"center", padding:16, color:C.muted, fontSize:12 }}>Cargando datos...</div>}
      <div style={S.page}>
        {tab===0&&<TabDashboard perfil={perfil} deudas={deudas} txns={txns} extras={extras}/>}
        {tab===1&&<TabRegistrar userId={userId} sesion={sesion} txns={txns} setTxns={setTxns}/>}
        {tab===2&&<TabDeudas userId={userId} sesion={sesion} deudas={deudas} setDeudas={setDeudas}/>}
        {tab===3&&<TabCobrar userId={userId} sesion={sesion} cobrar={cobrar} setCobrar={setCobrar}/>}
